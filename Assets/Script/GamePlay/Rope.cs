...
public class Rope : MonoBehaviour
{
    ...
    private Rigidbody2D endA_sj, endB_sj;

    [Header("Rope physics")]
    public int nr_segments = 2;
    ...
    void Start()
    {
        ...
        Material ropeMaterial = Resources.Load<Material>("Materials/Rope");
        if (ropeMaterial == null)
        {
            Debug.LogError("Rope material not found in Resources/Materials.");
            return;
        }

        // Assign the Rope material to the LineRenderer
        line.material = ropeMaterial;


        segmentPos = new Vector3[nr_segments + 1];

        if (!endA.GetComponent<Rigidbody2D>())
            endA_sj = endA.AddComponent<Rigidbody2D>();
        else endA_sj = endA.GetComponent<Rigidbody2D>();
        if (!endB.GetComponent<Rigidbody2D>())
            endB_sj = endB.AddComponent<Rigidbody2D>();
        else endB_sj = endB.GetComponent<Rigidbody2D>();
    ...
    private void Initialize()
    {
        ...
        for (int i = 1; i < nr_segments; i++)
        {
            ...
            cur_prev_sj.autoConfigureDistance = false;
            cur_succ_sj.autoConfigureDistance = false;

            cur_prev_sj.distance = 1.5f;
            cur_succ_sj.distance = 1.5f;
